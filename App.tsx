...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-100 font-sans">
      <header className="p-4 border-b border-gray-200 bg-white/80 backdrop-blur-md sticky top-0 z-10">
        <div className="container mx-auto max-w-6xl flex justify-between items-center">
          <div onClick={resetApp} className="cursor-pointer group">
            <h1 className="text-2xl font-bold text-gray-800 tracking-tight transition-colors group-hover:text-pink-500">
              {t('header.titlePart1')} <span className="text-pink-500 group-hover:text-gray-800 transition-colors">{t('header.titlePart2')}</span>
            </h1>
          </div>
          <div className='flex items-center gap-4'>
            <button 
                onClick={resetApp} 
                className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-500 rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 transition-colors"
              ...
              <div className="space-y-4 sticky top-24">
                ...
                {viewMode === 'combine' && (
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-indigo-500 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-600 transition-all duration-300 disabled:bg-indigo-300 disabled:cursor-not-allowed transform hover:scale-105"
                    >
                        <SparklesIcon />
                        {isLoading ? t('main.combining') : t('main.combine')}
                    </button>
                )}
            ...
            <div className="md:w-2/3">
                ...
                {isLoading && viewMode === 'combine' && (
                    <div className="flex flex-col items-center justify-center h-full min-h-[50vh] text-center">
                        <Loader />
                        <p className="mt-4 text-gray-600 font-medium">{loadingMessage || t('main.combining')}</p>
                    </div>
                )}
                ...
                {!isLoading && (
                    <>
                        {viewMode === 'single' && !selectedItem && (
                            <div className="flex items-center justify-center h-full min-h-[50vh] text-center p-8 bg-white rounded-lg shadow-sm">
                                <p className="text-gray-500">{t('main.selectItemPrompt')}</p>
                            </div>
                        )}
                        {viewMode === 'combine' && combinationSelection.length < 2 && (
                            <div className="flex items-center justify-center h-full min-h-[50vh] text-center p-8 bg-white rounded-lg shadow-sm">
                                <p className="text-gray-500">{t('main.combinePrompt')}</p>
                            </div>
                        )}
                        ...
                        {viewMode === 'combine' && combinationResults.length > 0 && (
                            <div className="space-y-8">
                                <h2 className="text-xl font-semibold text-gray-700">{t('main.combinationSuggestions')}</h2>
