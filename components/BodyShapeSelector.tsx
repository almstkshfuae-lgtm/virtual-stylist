...
export const BodyShapeSelector: React.FC<BodyShapeSelectorProps> = ({ selectedShape, onShapeChange }) => {
    ...
    return (
        <div className="p-4 bg-white rounded-lg border border-gray-200">
            <h3 className="text-sm font-medium text-gray-700">{t('bodyShape.title')}</h3>
                ...
                {BODY_SHAPES.map((shape) => {
                    ...
                    return (
                        <div key={shape} className="relative group">
                            ...
                            <div
                                role="tooltip"
                                className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 p-2 text-xs text-white bg-gray-800 rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10"
                            >
