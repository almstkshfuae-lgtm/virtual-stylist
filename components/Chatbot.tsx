...
export const Chatbot: React.FC<ChatbotProps> = ({ isOpen, onClose, history, onSendMessage, isLoading, selectedItemImage }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 sm:bottom-8 sm:right-8 z-20">
      <div className="w-[calc(100vw-3rem)] sm:w-96 h-[60vh] sm:h-[32rem] bg-white rounded-xl shadow-2xl flex flex-col transition-transform duration-300 transform origin-bottom-right">
        {/* Header */}
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          {selectedItemImage && (
            <div className="mb-4 p-2 bg-gray-100 rounded-lg flex items-center gap-2 text-xs text-gray-600">
          ...
          {history.map((msg, index) => (
            <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} mb-3`}>
              <div className={`max-w-xs lg:max-w-sm px-4 py-2 rounded-2xl ${msg.role === 'user' ? 'bg-pink-500 text-white' : 'bg-gray-200 text-gray-800'}`}>
        ...
        <div className="p-4 border-t border-gray-200">
          <form onSubmit={handleSend} className="flex items-center gap-2">
            <input
              ...
              className="flex-1 w-full px-4 py-2 text-sm bg-gray-100 border border-transparent rounded-full focus:outline-none focus:ring-2 focus:ring-pink-500"
            />
            <button
              type="submit"
