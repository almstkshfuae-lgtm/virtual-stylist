...
const isAllowedRole = (value: string): value is Exclude<CleanContent['role'], undefined> =>
...
const rejectUnknownKeys = (
  ...
  const unknown = Object.keys(obj).filter((key) => !Array.from(allowedKeys).includes(key));
  if (unknown.length > 0) {
...
const sanitizeContent = (raw: unknown): ValidationResult<CleanContent> => {
  ...
  const roleStr = cleanString(obj.role, 10);
  if (roleStr && !isAllowedRole(roleStr)) {
    return { error: 'content.role is invalid' };
  }

  if (roleStr && isAllowedRole(roleStr)) {
    return { value: { role: roleStr, parts } };
  }
  return { value: { parts } };
};
...
const logEvent = (event: 'proxy.request.start' | 'proxy.request.success' | 'proxy.request.error' | 'proxy.rejected.method' | 'proxy.rejected.validation' | 'proxy.rejected.unauthorized' | 'proxy.rejected.rate_limit' | 'proxy.error.config', fields: Record<string, unknown>) => {
